...
class DGLGNNTrainer():
    def __init__(self, cfg, feature_type):
        ...
        # ! Load data
        dataset, _, features,train_mask, val_mask, test_mask  = load_data(self.dataset_name,feature_type = self.feature_type, seed = cfg.seed, split = cfg.split, use_dgl=True)
        if 'Amazon' in self.dataset_name:
            data = dataset
        else:
            data = dataset[0]
        ...
        self.y = data.ndata['label'].squeeze().to(self.device).long()
        ...
        if self.gnn_model_name == "RevGAT":
            ...
            self.model = RevGAT(in_feats=768,

...
class ElementWiseLinear(nn.Module):
    def __init__(self, size, weight=True, bias=True, inplace=False):
        ...
        if bias:
            ...
            self.bias = nn.Parameter(torch.zeros(size, dtype=torch.float32))  # or torch.float64 if needed
        else:
            self.bias = None
...
class RevGAT(nn.Module):
    def __init__(
        ...
        super().__init__()
        print("n_classes:", n_classes, "n_hidden:", n_hidden)  # 调试输出
        ...
        for i in range(n_layers):
            ...
            print("Layer:", i, "in_hidden:", in_hidden, "out_hidden:", out_hidden)  # 调试输出
